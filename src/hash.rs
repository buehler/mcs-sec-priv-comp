...
pub(crate) fn create_bins(points: &[Point], delta: u128) -> HashSet<Point> {
...
pub(crate) fn create_bin(point: &Point, delta: u128) -> Point {
...
mod tests {
    ...
    #[test]
    fn test_create_bin() {
        let point = Point::new(1u128, 8u128);
        let delta = 2;
        let bin = create_bin(&point, delta);
        assert_eq!(bin, Point::new(1u128, 2u128));
    }

    #[test]
    fn test_create_bins() {
        let points = vec![Point::new(1u128, 8u128), Point::new(1u128, 12u128)];
        let delta = 2;
        let bins = create_bins(&points, delta);
        let expected_bins: HashSet<Point> =
            vec![Point::new(1u128, 2u128), Point::new(1u128, 3u128)]
                .into_iter()
                .collect();
    ...
    fn test_points_in_same_bin() {
        let point1 = Point::new(1u128, 8u128);
        let point2 = Point::new(1u128, 9u128);
        let delta = 2;
