...
use crate::okvs::{Okvs, OkvsKey, OkvsValue, Pair};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let okvs_a = okvs::RbOkvs::new(okvs_a_length);
    let mut s_a = Vec::new();
    ...
    for bin in hash::create_bins(&items_party_a, distance_threshold) {
        ...
        s_a.push((OkvsKey(bin.to_ne_bytes()), OkvsValue(m_b.to_ne_bytes())))
    }
    while s_a.len() < okvs_a_length {
        s_a.push((
            OkvsKey(rng.next_u64().to_ne_bytes()),
            OkvsValue(rng.next_u64().to_ne_bytes()),
        ));
    }
    ...
    let okvs_b = okvs::RbOkvs::new(okvs_b_length);
    let mut s_b = Vec::new();
    for bin in hash::create_bins(&items_party_b, distance_threshold) {
        let bin_key = OkvsKey(bin.to_ne_bytes());
        ...
        if val_from_a == point {
            let m_b = point;
            s_b.push((OkvsKey(bin.to_ne_bytes()), OkvsValue(m_b.to_ne_bytes())))
        }
    }
